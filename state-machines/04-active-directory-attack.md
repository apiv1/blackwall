...
### 1. [bloodhound-python](../tools/bloodhound-python.md) - AD 关系图谱收集
...
- **如果破解成功** → 可能获得高权限账户，转移到：[权限提升](05-privilege-escalation.md)
- **如果无 SPN** → 转移到：其他攻击方式

---

### 4. impacket-secretsdump - [凭据提取](06-credential-extraction.md)
...
- **如果连接成功** → 获得 shell，转移到：
  ...
  - 横向移动
  - [持久化](11-post-exploitation-persistence.md)
- **如果连接失败** → 尝试其他方法或目标
...
- **如果伪造成功** → 导出票据
  - 使用票据访问任何资源
  - [持久化](11-post-exploitation-persistence.md)访问
- **如果伪造失败** → 检查哈希和 SID
...
[步骤 5：持久化]
    黄金票据:
        ...
        [持久化](11-post-exploitation-persistence.md)域管理员访问
...
8. **状态机判定**：获得域控制器完全控制权
   - 转移到：[凭据提取](06-credential-extraction.md)和持久化
...
完成 AD 攻击后，根据结果转移到：
1. **[权限提升](05-privilege-escalation.md)状态机**（如果获得低权限 shell）
