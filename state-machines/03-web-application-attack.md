...
### 1. [whatweb](../tools/whatweb.md) - Web 指纹识别
...
### 2. [nikto](../tools/nikto.md) - Web 服务器漏洞扫描
...
### 3. [ffuf](../tools/ffuf.md) - 高速目录/文件爆破
...
### 4. [gobuster](../tools/gobuster.md) - 多功能爆破工具
...
### 5. [wpscan](../tools/wpscan.md) - WordPress 专用扫描器
...
- **如果发现用户名** → 转移到：[暴力破解](09-brute-force-attack.md)
- **如果发现漏洞** → 转移到：[漏洞利用](12-exploitation.md)

---

### 6. [sqlmap](../tools/sqlmap.md) - SQL 注入自动化工具
...
**状态转移 (State Transition)**：
- **如果发现注入点** → 转移到：[数据库枚举](13-database-attack.md)
- **如果枚举到数据** → 转移到：数据提取和分析
- **如果获取到凭据** → 转移到：登录尝试
- **如果无注入** → 转移到：其他漏洞测试
...
### 7. [burpsuite](../tools/burpsuite.md) - Web 应用安全测试平台
...
### 8. [commix](../tools/commix.md) - 命令注入自动化工具
...
- **如果获取 shell** → 转移到：[权限提升](05-privilege-escalation.md)
- **如果无注入** → 转移到：其他漏洞测试

---

### 9. [wafw00f](../tools/wafw00f.md) - WAF 识别
...
[步骤 5：深度利用]
    IF SQL 注入成功:
        枚举数据库 → 提取数据 → 获取凭据
    IF 命令注入成功:
        获取 shell → [权限提升](05-privilege-escalation.md)
    IF 文件上传成功:
...
4. **状态机判定**：找到 exploit
   - 转移到：[漏洞利用](12-exploitation.md)

5. **第三步：[漏洞利用](12-exploitation.md)**
   使用 exploit 上传 webshell
   **结果**：上传成功，获取 shell

6. **状态机判定**：获取 webshell 访问权限
   - 转移到：[权限提升](05-privilege-escalation.md)
...
4. **状态机判定**：确认 MySQL 注入
   - 转移到：[数据库枚举](13-database-attack.md)
...
- 降低扫描速度
- 使用随机 User-Agent
- 使用[代理](14-tunneling-pivoting.md)池
- 编码 payload
...
2. **[数据库攻击](13-database-attack.md)状态机**（如果发现数据库）
3. **横向移动状态机**（如果在内网）
4. **[凭据提取](06-credential-extraction.md)状态机**（如果获得系统访问）
